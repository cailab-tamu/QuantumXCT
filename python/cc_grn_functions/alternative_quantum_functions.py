...
def add_bidirectional_gates_and_measurements(
    base_circuit: QuantumCircuit,
    circ1_num_qubits: int,
    interaction_pair_sequence: list[frozenset]
) -> QuantumCircuit:
    """
    Applies a sequence of bidirectional interaction pairs to the circuit.
    Each interaction pair {q1, q2} is "unfolded" into two CRX(pi/2) gates.

    Args:
        base_circuit: The circuit containing the initial states.
        circ1_num_qubits: The number of qubits in the first logical register.
        interaction_pair_sequence: A list of frozensets, where each frozenset contains
                                   two qubit indices representing an interaction.
                           
    Returns:
        QuantumCircuit: A new circuit with the bidirectional gates and measurements added.
    """
    circuit_with_gates = base_circuit.copy()
    qr_all = circuit_with_gates.qregs[0]
    cr_measure1 = circuit_with_gates.cregs[0]
    cr_measure2 = circuit_with_gates.cregs[1]
    
    for pair in interaction_pair_sequence:
        # Ensure it's a valid pair before unpacking
        if len(pair) != 2:
            raise ValueError(f"Interaction pairs must contain exactly two qubit indices. Got: {pair}")
        
        q1, q2 = tuple(pair)
        
        # --- UNFOLDING STEP ---
        # Add the CRX(pi/2) gate in both directions
        circuit_with_gates.crx(np.pi / 2, qr_all[q1], qr_all[q2])
        circuit_with_gates.crx(np.pi / 2, qr_all[q2], qr_all[q1])

    # Add measurements after all gates are applied
    circuit_with_gates.measure(qr_all[0:circ1_num_qubits], cr_measure1)
    circuit_with_gates.measure(qr_all[circ1_num_qubits:circuit_with_gates.num_qubits], cr_measure2)
    
    return circuit_with_gates
...
def create_interaction_pairs_from_locations(
    locations_A: list[int],
    locations_B: list[int]
) -> set[frozenset]:
    """
    Generates a set of unique bidirectional interaction pairs from two lists of qubit locations.
    Each interaction is represented by a frozenset of two qubit indices.
    ...
    Returns:
        set[frozenset]: A set of unique frozensets, e.g., {frozenset({0, 4}), frozenset({1, 5})}.
    """
    interaction_pairs = set()
    # Generate interactions between qubits in group A and group B
    for q_a in locations_A:
        for q_b in locations_B:
            if q_a != q_b:
                # A frozenset is used because it is hashable (can be in a set)
                # and automatically treats {q_a, q_b} and {q_b, q_a} as identical.
                interaction_pairs.add(frozenset({q_a, q_b}))
...
# Make sure _process_target_state_input and create_interaction_pairs_from_locations are defined

def find_bidirectional_interaction_candidates(
    state_probs_initial1: Union[dict, list, np.ndarray],
    state_probs_initial2: Union[dict, list, np.ndarray],
    state_vec_probs_target1: Union[dict, list, np.ndarray],
    state_vec_probs_target2: Union[dict, list, np.ndarray],
    ng_circ1: int, # Crucial: Size of the first logical register
    threshold: float,
    search_mode: str = "abs",
    plot_filename: Optional[str] = None,
    show_plot: bool = False,
    verbose_print: bool = False
) -> Tuple[List[frozenset], Dict[str, float]]:
    """
    Generates a list of bidirectional interaction pair candidates (frozensets)
    based on density matrix differences.
    """
    # --- 1. Process Inputs and build global states ---
    state_list1_initial, _ = _process_target_state_input(state_probs_initial1)
    state_list2_initial, _ = _process_target_state_input(state_probs_initial2)
    state_list1_target, _ = _process_target_state_input(state_vec_probs_target1)
    state_list2_target, _ = _process_target_state_input(state_vec_probs_target2)
    
    if state_list1_initial.size == 0 or state_list2_initial.size == 0:
        return [], {}
    ...
    stats = {
        "max_element": np.max(real_part_data),
        "min_element": np.min(real_part_data),
        "abs_average": np.mean(np.abs(real_part_data)),
    }

    # --- 4. Generate Bidirectional Interaction Pairs ---
    all_interaction_configurations = set()
    ...
    for r_idx, c_idx in zip(rows, cols):
        ...
        all_involved_qubits = qubits_row.union(qubits_col)
        
        # Separate the involved qubits into the two cell types
        qubits_group_A = {q for q in all_involved_qubits if q < ng_circ1}
        qubits_group_B = {q for q in all_involved_qubits if q >= ng_circ1}

        # Create the unique interaction pairs between the two groups
        if qubits_group_A and qubits_group_B:
            # Use the new function to generate frozensets
            new_pairs = create_interaction_pairs_from_locations(list(qubits_group_A), list(qubits_group_B))
            all_interaction_configurations.update(new_pairs)
            
    # Sort for consistent output. The key sorts the frozenset as a tuple of its sorted elements.
    interaction_candidates = sorted(list(all_interaction_configurations), key=lambda f: tuple(sorted(f)))
    
    # --- 5. Optional Plotting ---
    if plot_filename or show_plot:
        fig, ax = plt.subplots(figsize=(7, 6))
        sns.heatmap(real_part_data, annot=False, cmap="RdBu", cbar=True, ax=ax, xticklabels=labels, yticklabels=labels)
        ax.set_title(f"Real Part of DM Difference (Mode: {search_mode})")
        plt.tight_layout()
        if plot_filename: plt.savefig(plot_filename)
        if show_plot: plt.show()
        plt.close(fig)
    ...
    # --- 6. Verbose Printing ---
    if verbose_print:
        total_possible = ng_circ1 * (num_qubits - ng_circ1)
        print(f"\n--- Bidirectional Interaction Analysis ({search_mode}) ---")
        print(f"Max Signal: {stats['max_element']:.4f}")
        print(f"Candidates Found: {len(interaction_candidates)} / {total_possible}")
        print("------------------------------\n")
        
    return interaction_candidates, stats
...
def _run_single_greedy_search_from_start(
    # NEW: Pass the builder function as the first argument
    circuit_builder: Callable[[List[Union[Tuple[int, int], frozenset]]], QuantumCircuit],
    all_candidates: List[Union[Tuple[int, int], frozenset]],
    starting_candidate: Union[Tuple[int, int], frozenset],
    state_vec_probs_target1: Union[dict, list, np.ndarray],
    state_vec_probs_target2: Union[dict, list, np.ndarray],
    nshots: int,
    kl_tol: float,
    initial_kl_for_path: float,
    max_greedy_depth: int = 30,
    min_cnot_depth: int = 1 # Retaining this parameter
):
    """
    [REFACTORED HELPER] Runs one greedy forward search path from a starting candidate.
    This function is now generic and uses a provided circuit_builder.

    Args:
        circuit_builder (Callable): A function that takes a sequence of candidates
                                    and returns a fully constructed QuantumCircuit.
        all_candidates (List): The full list of candidates to search from (can be tuples or frozensets).
        starting_candidate: The first candidate to seed the sequence.
        ... (other parameters) ...
    """
    
    # Initialize the sequence with the starting candidate
    best_sequence = [starting_candidate]
    
    # Use the pre-calculated score passed from the main function
    best_sequence_on_path = list(best_sequence)
    best_kl_on_path = initial_kl_for_path
    current_kl_sum_on_path = initial_kl_for_path
    
    # Initialize history to record the entire path taken
    path_history = [(initial_kl_for_path, list(best_sequence))]
    
    improvement_made = True
    while improvement_made:
        improvement_made = False
        best_next_candidate = None
        min_kl_this_iteration = current_kl_sum_on_path
        
        # Find the single best candidate to add
        for candidate in all_candidates:
            if candidate not in best_sequence:
                trial_sequence = best_sequence + [candidate]
                
                # --- USE THE PASSED-IN BUILDER FUNCTION ---
                trial_circuit = circuit_builder(trial_sequence)
                
                # Score the new circuit
                kl_div1, kl_div2 = score_circuit_kl_divergences(
                    trial_circuit,
                    state_vec_probs_target1,
                    state_vec_probs_target2,
                ...
                if kl_div1 is not None and kl_div2 is not None:
                    current_kl_sum = kl_div1 + kl_div2
                    if current_kl_sum < min_kl_this_iteration:
                        min_kl_this_iteration = current_kl_sum
                        best_next_candidate = candidate
                        improvement_made = True
                        
        # If a valid improvement was found, update the sequence and continue
        if improvement_made:
            best_sequence.append(best_next_candidate)
            current_kl_sum_on_path = min_kl_this_iteration
            path_history.append((current_kl_sum_on_path, list(best_sequence)))
        
        # --- Check termination conditions ---
        if len(best_sequence) >= max_greedy_depth:
             print(f"  Max depth ({max_greedy_depth}) reached. Stopping this search path.")
             break
        
        # Break if no improvement was found in the entire loop
        if not improvement_made:
            break
            
        # Update the best-ever sequence found along this specific path
        if current_kl_sum_on_path < best_kl_on_path:
            best_kl_on_path = current_kl_sum_on_path
            best_sequence_on_path = list(best_sequence)
            print(f"    --> Path improved: New best KL on this path is {best_kl_on_path:.6f} with {len(best_sequence_on_path)} candidates.")

        # Early stopping if target KL is met
...
# ==============================================================================
# --- REFACTORED HELPER FUNCTIONS for Algorithm 2 ---
# ==============================================================================

def _run_single_greedy_search_from_start_refactored(
    ...
    kl_tol: float,
    initial_kl_for_path: float,
    max_greedy_depth: int,
    min_cnot_depth: int
) -> Tuple[List[Tuple[float, list]], list, float]:
    """
    [REFACTORED HELPER] Runs one greedy forward search path from a starting candidate.
    Uses a provided circuit_builder function to remain generic.
    """
    best_sequence = [starting_candidate]
    best_sequence_on_path = list(best_sequence)
    best_kl_on_path = initial_kl_for_path
...
def _run_greedy_removal_search_refactored(
    circuit_builder: Callable,
    ...
    nshots: int,
    kl_tol: float,
    min_cnot_depth: int = 0
) -> Tuple[List[Tuple[float, list]], list, float]:
    """
    [REFACTORED HELPER] Runs an iterative greedy removal search on a sequence.
    """
    best_sequence = list(initial_sequence)
    best_sequence_on_path = list(initial_sequence)
    best_kl_on_path = initial_kl
    current_kl_sum_on_path = initial_kl
    removal_history = []

    while len(best_sequence) > min_cnot_depth:
        improvement_made = False
        best_index_to_remove = -1
        best_kl_after_removal = current_kl_sum_on_path
        
        for i in range(len(best_sequence)):
            trial_sequence = best_sequence[:i] + best_sequence[i+1:]
            trial_circuit = circuit_builder(trial_sequence)
            kl_divs = score_circuit_kl_divergences(trial_circuit, state_vec_probs_target1, state_vec_probs_target2, nshots)
            
            if kl_divs and None not in kl_divs:
                current_kl_after_removal = sum(kl_divs)
                if current_kl_after_removal < (best_kl_after_removal - (kl_tol * 0.3)):
                    best_kl_after_removal = current_kl_after_removal
        ...
        if improvement_made:
            best_sequence.pop(best_index_to_remove)
            current_kl_sum_on_path = best_kl_after_removal
            removal_history.append((current_kl_sum_on_path, list(best_sequence)))
...
def find_best_cnot_sequence_multi_epoch(
    ...
    state_vec_probs_target1: dict,
    state_vec_probs_target2: dict,
    cnot_search_candidates: Optional[List[Union[Tuple[int, int], frozenset]]] = None,
    n_epochs: int = 10,
    nshots: int = 1000,
    threshold: float = 0.1,
    min_cnot_depth: int = 1,
    search_mode: str = "abs", # pos, neg, abs
    search_unit: str = "directional", # directional, bidirectional
    kl_tol: float = 0.05,
    max_greedy_depth: int = 30
):
    """
    [FINAL CORRECTED] Performs a multi-epoch greedy search with CNOT removal refinement,
    supporting both directional gates and bidirectional interaction pairs.
    """
    ng_circ1 = circ1.num_qubits
    print(f"\n--- Starting Multi-Epoch Refined Search (Mode: {search_unit}) ---")
    start_total_time = time.time()
    
    # --- 1. CANDIDATE GENERATION ---
    if cnot_search_candidates is not None:
        print("\n--- Using provided candidate list for the search. ---")
        search_candidates = cnot_search_candidates
    else:
        print("\n--- Generating candidates from density matrix difference. ---")
        if search_unit == 'directional':
            search_candidates, _ = find_cnot_candidates_from_state_diff(
        ...
        elif search_unit == 'bidirectional':
            search_candidates, _ = find_bidirectional_interaction_candidates(
                state_probs_initial1, state_probs_initial2,
                state_vec_probs_target1, state_vec_probs_target2,
                ng_circ1=circ1.num_qubits, 
                threshold=threshold, search_mode=search_mode, verbose_print=True
            ...
            raise ValueError("search_unit must be 'directional' or 'bidirectional'")
    print(f"Number of candidates to be searched: {len(search_candidates)}")

    # --- 2. BASELINE CALCULATION ---
    base_combined_circuit = concatenate_circuits_with_separate_measurements(circ1, circ2)
    base_circuit_with_measurements = add_cnots_and_measurements_to_circuit(base_combined_circuit, ng_circ1, [])
    kl_divs_baseline = score_circuit_kl_divergences(base_circuit_with_measurements, state_vec_probs_target1, state_vec_probs_target2, nshots)
    initial_kl_sum = sum(kl_divs_baseline) if kl_divs_baseline and None not in kl_divs_baseline else float('inf')
    print(f"Initial KL divergence (baseline): {initial_kl_sum:.6f}")
    
    # --- 3. INITIALIZE SEARCH ---
    evaluation_history = [(initial_kl_sum, [])]
    best_overall_sequence = []
    best_overall_kl_sum = initial_kl_sum

    if not search_candidates:
        print("\nNo candidates found or provided. Skipping search epochs.")
    else:
        # --- 4. MULTI-EPOCH EXPLORATION ---
        num_epochs_to_run = min(n_epochs, len(search_candidates))
        shuffled_candidates = search_candidates.copy()
        random.shuffle(shuffled_candidates)
        
        # Define the unified builder function that captures necessary variables
        def build_trial_circuit(sequence):
            if search_unit == 'directional':
                return add_cnots_and_measurements_to_circuit(base_combined_circuit, ng_circ1, sequence)
            else: # bidirectional
                return add_bidirectional_gates_and_measurements(base_combined_circuit, ng_circ1, sequence)

        for epoch in range(num_epochs_to_run):
            starting_candidate = shuffled_candidates[epoch]
            
            trial_circuit = build_trial_circuit([starting_candidate])
            temp_kl_scores = score_circuit_kl_divergences(trial_circuit, state_vec_probs_target1, state_vec_probs_target2, nshots)
            
            if temp_kl_scores and None not in temp_kl_scores:
                temp_kl_sum = sum(temp_kl_scores)
                print(f"\n--- Starting Epoch {epoch + 1}/{num_epochs_to_run} with {starting_candidate} (KL: {temp_kl_sum:.6f}) ---")
            else:
                continue
            ...
            path_hist, best_sequence_this_epoch, min_kl_this_epoch = _run_single_greedy_search_from_start_refactored(
                circuit_builder=build_trial_circuit,
                all_candidates=search_candidates,
            ...
            if min_kl_this_epoch < best_overall_kl_sum:
                ...
                if best_overall_sequence:
                    print("\n--- Starting Greedy CNOT Removal Search on New Best Sequence ---")
                    rem_hist, best_seq_after_removal, best_kl_after_removal = _run_greedy_removal_search_refactored(
                        circuit_builder=build_trial_circuit,
                        initial_sequence=best_overall_sequence,
                        initial_kl=best_overall_kl_sum, 
                        state_vec_probs_target1=state_vec_probs_target1,
                        state_vec_probs_target2=state_vec_probs_target2,
                        nshots=nshots,
                        kl_tol=kl_tol,
                    ...
                    if best_kl_after_removal < best_overall_kl_sum:
                        ...
                        best_overall_sequence = best_seq_after_removal
                        print(f"\n--> Removal search found a new overall best KL Sum: {best_overall_kl_sum:.6f}")

    # --- 5. FINAL SELECTION (OCCAM'S RAZOR) ---
    ...
    # ... (Final print statements and return) ...
