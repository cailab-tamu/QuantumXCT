...
figure;
nexttile
    C.Gates(end-3).Angles = optimtheta(1);
    C.Gates(end-2).Angles = optimtheta(2);
    C.Gates(end-1).Angles = optimtheta(3);
    C.Gates(end).Angles = optimtheta(4);
plot(C);
title(sprintf(['KL = %.3f\n(t_1 = %.3f, t_2 = %.3f, ' ...
    ' t_3 = %.3f t_4 = %.3f)'], ...
    fval, optimtheta(1), optimtheta(2),...
    optimtheta(3), optimtheta(4)));
...
function [y] = i_obj(theta, pt_f, pt_c, C)
    % N = 7;
    C.Gates(end-3).Angles = theta(1);
    C.Gates(end-2).Angles = theta(2);
    C.Gates(end-1).Angles = theta(3);
    C.Gates(end).Angles = theta(4);
    S = simulate(C);    
