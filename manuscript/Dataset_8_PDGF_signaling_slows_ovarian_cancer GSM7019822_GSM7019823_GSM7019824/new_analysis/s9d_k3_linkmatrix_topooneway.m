...
for idx = 1:numComb
    ...
    combinedGate = [cg1_mapped; cg2_mapped; cxGate(5, 4); layer_inte];
    C = quantumCircuit(combinedGate);
    Cc{idx} = C;
    S = simulate(C);
    ...
    if all(ismember(sort(configsK{idx},2),targettop,"rows"))
        idealY = [idealY; Y(idx)];
        isideal(idx) = true;
    end
...
for topk = 1:3
    ...
    xlabel('Expression pattern');
    title('Fibroblasts')
    title(t, sprintf('Numerically Best Configuration, KL = %f', Y(b(topk))));
end
...
for topk = 1:numComb
    ...
    for k = 1:length(configsK{topk})
        x = configsK{topk}(k, 1) +" "+ FCGenes(configsK{topk}(k, 1))+...
