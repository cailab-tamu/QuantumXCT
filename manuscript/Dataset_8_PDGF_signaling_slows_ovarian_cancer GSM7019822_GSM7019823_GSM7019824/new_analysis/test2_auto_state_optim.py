...
def calculate_cost(thetas, qc1, qc2, shots=10000):
    """
    Calculate the cost between measured frequencies and target frequencies.
    ...
    Returns:
        float: Total cost (MSE between measured and target frequencies)
    """
    ...
    measured_f3 = np.array([first3_freq[format(i, '03b')] for i in range(8)])
    measured_c4 = np.array([last4_freq[format(i, '04b')] for i in range(16)])
    
    # Calculate mean squared error
    mse_f3 = np.mean((measured_f3 - pt_f_co) ** 2)
    mse_c4 = np.mean((measured_c4 - pt_c_co) ** 2)
    
    # Total cost is the sum of both MSEs
    total_cost = mse_f3 + mse_c4
