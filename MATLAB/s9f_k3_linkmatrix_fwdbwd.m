...
[pt_f_mo, ~, ~, ~] = getn(batquery, genquery, sce, false);

batquery = "Cancer Cells (Mo)"; genquery = CancerGenes;
[pt_c_mo, ~, ~, ~] = getn(batquery, genquery, sce, false);
...
cg_f1 = initGate(1:3, sqrt(pt_f_mo));          % creates CompositeGate to initialize qubits 1–3 :contentReference[oaicite:1]{index=1}
qc_f1 = quantumCircuit(cg_f1);        % initialize circuit with the gate

cg_c2 = initGate(1:4, sqrt(pt_c_mo));          % creates CompositeGate to initialize qubits 1–4 :contentReference[oaicite:1]{index=1}
qc_c2 = quantumCircuit(cg_c2);        % initialize circuit with the gate

cg_f3 = initGate(1:3, sqrt(pt_f_co));          % creates CompositeGate to initialize qubits 1–3 :contentReference[oaicite:1]{index=1}
qc_f3 = quantumCircuit(cg_f3);        % initialize circuit with the gate

cg_c4 = initGate(1:4, sqrt(pt_c_co));          % creates CompositeGate to initialize qubits 1–4 :contentReference[oaicite:1]{index=1}
qc_c4 = quantumCircuit(cg_c4);        % initialize circuit with the gate

%%
cg1_mapped = compositeGate(cg_f1, [1 2 3]);
cg2_mapped = compositeGate(cg_c2, [4 5 6 7]);

cg3_mapped = compositeGate(cg_f3, [1 2 3]);
cg4_mapped = compositeGate(cg_c4, [4 5 6 7]);
...
for idx = 1:numComb
    ...
    layer_inte = [];
    for k = 1:height(configsK{idx})
        layer_inte = [layer_inte; cxGate(configsK{idx}(k,2), configsK{idx}(k,1))];
    end
    combinedGate = [cg3_mapped; cg4_mapped; cxGate(5,4); layer_inte];
    C = quantumCircuit(combinedGate);
    Cc{idx, 2} = C;
    S = simulate(C);
    [~, po_f] = querystates(S,[1 2 3]);     % observed state pattern in fibroblast
    [~, po_c] = querystates(S,[4 5 6 7]);   % observed state pattern in cancer   
    kl3 = i_kldiverg(pt_f_mo, po_f);
    kl4 = i_kldiverg(pt_c_mo, po_c);

    Y(idx) = kl1 + kl2 + kl3 + kl4;
...
%%
for topk = 1:3
    t = fun_drawreshisto(b(topk), Cc(:,1), configsK, ...
...
a = a(idx);
for topk = 1:length(a)
    t = fun_drawreshisto(a(topk), Cc(:,1), configsK, ...
        pt_c_mo, pt_c_co, pt_f_mo, pt_f_co, ...
        FCGenes, states_c, states_f, targettop);
